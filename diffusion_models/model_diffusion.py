...
class social_transformer(nn.Module):
	def __init__(self, input_dim=48):
		...
		# input_dim=48 for eth, 60 for nba
		self.encode_past = nn.Linear(input_dim, 256, bias=False)
...
class TransformerDenoisingModel(nn.Module):
	...
	def __init__(self, context_dim=256, tf_layer=2, input_dim=48):
		super().__init__()
		self.encoder_context = social_transformer(input_dim=input_dim)
	...
	def generate_accelerate(self, x, beta, context, mask):
		# [35, 10, 12, 2], [35, 1, 1, 1], [35, 8, 6], [35, 35]
		...
		# ctx_emb: 11, 10, 1, 259
		pred_len = x.size(2)
		x = self.concat1.batch_generate(ctx_emb, x).contiguous().view(-1, pred_len, 512)
		# print(x.size(),'----yyy')
		...
		trans = self.transformer_encoder(final_emb).permute(1, 0, 2).contiguous().view(-1, 10, pred_len, 512)
		# trans: 11, 10, 20, 512
		trans = self.concat3.batch_generate(ctx_emb, trans)
		trans = self.concat4.batch_generate(ctx_emb, trans)

		# tmp = self.linear.batch_generate(ctx_emb, trans)
		# print(tmp.size())
