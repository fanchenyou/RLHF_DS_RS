...
class LEDInitializer(nn.Module):
	def __init__(self, t_obs=8, s=24, n=20):
...
class LEDInitializerWithValueHead(LEDInitializer):
	def __init__(self, t_obs=8, s=24, n=20):
		super(LEDInitializerWithValueHead, self).__init__(t_obs, s, n)
		self.value_head_dim = s * n // 2
		self.value_head = MLP(768, self.value_head_dim, hid_feat=(256, 256), activation=nn.ReLU())
		# self.corr_decoder = MLP(256*2+32, n * 3 * s // 2, hid_feat=(1024, 1024), activation=nn.ReLU())
	...
	def forward(self, x, mask=None):
		...
		val_total = torch.cat((ego_mean_embed, ego_var_embed, social_embed), dim=-1)
		# print(val_total.size(),'----',var_total.size())
		value = self.value_head(val_total).reshape(self.n, x.size(0), self.fut_len)
		#corr = self.value_head(var_total).reshape(x.size(0), self.n, self.fut_len, 3)

		return guess_var, guess_mean, guess_scale, value
